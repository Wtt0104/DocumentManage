layui.define(['jsHelper'], function (exports) {
    ...
    if (typeof ($.lrtree) == undefined || $.lrtree == null) {
        layui.link(layui.cache.base + 'lp/jsTree/jsTree.css');
        (function(a, b) {
            a.lrtree = {
                ...
                resetJson: function (list,dataList) {
                   $.each(list,function (index,lists) {
                       var hasChildren=false;
                       if(lists.LP_Icon==''){
                           lists.LP_Icon=null
                       }
                       if(lists.LP_Tables.length>0){
                           hasChildren=true;
                           // a.lrtree.resetJson(lists.LP_Tables,dataList)
                       }
                        dataList.push({
                            id: lists.LP_Id,
                            text: lists.LP_Name,
                            ...
                            ChildNodes:a.lrtree.resetJson(lists.LP_Tables,dataList)
                        });
                       return dataList
                   }) 
                }
            ...
            a.fn.lrtree = function(g) {
                ...
                if (d.url) {
                    b.http.httpAsync(d.method, d.url, d.param, function(h) {
                        var list=h.DataPacket.list;
                        var dataList=[];
                        console.log(a.lrtree.resetJson(list,dataList));

                        // $.each(list,function (index, lists) {
                        //
                        // });
                        c[0]._lrtree.dfop.data = h.DataPacket.list || [];
